...
class CI_Calendar {
	...
	/**
	 * Generate the calendar for use in the Academic Setup > Weeks page
	 *
	 * @access	public
	 ...
	 * @return	string
	 */
	function generate($year = '', $month = '', $data = array(), $weekdates = NULL, $week_id = NULL)
	{
		...
		$colspan = ($this->show_next_prev == TRUE) ? 5 : 8;
		...
		while ($day <= $total_days)
		{
			...
			// CR 2009-01-24
			// This is the place-holder for our tickbox.
			// We replace it later if we had a monday date in this week
			$out .= '<td>{tickbox}</td>';
			$crbs_mon = 0;
			...
			for ($i = 0; $i < 7; $i++)
			{
				...
				// CR 2009-01-24
				// If we have a valid monday of this month week, set the tickbox and flag
				#$crbs_date = "$year-$month-$day";
				$crbs_date = mktime(0, 0, 0, $month, $day, $year);
				$crbs_daynum = date("N", $crbs_date);
				/* $out .= "[$crbs_daynum]";
				$out .= "^$day^";
				$out .= "(" . date("N", strtotime("$year-$month-$day")) . ")"; */
				if($crbs_daynum == 1 && $day > 0){
					$crbs_mondate = date("Y-m-d", $crbs_date);
					$check['name'] = 'dates[]';
					$check['value'] = $crbs_mondate;
					$check['id'] = "chk_{$crbs_mondate}";
					$check['checked'] = ($weekdates != NULL && array_key_exists($crbs_mondate, $weekdates) && $weekdates[$crbs_mondate] == $week_id);
					$check['class'] = 'check';
					$crbs_tick = form_checkbox($check);
					#$crbs_tick = '<input type="checkbox" id="chk_' . $crbs_mondate . '" name="dates[]" value="'.$crbs_mondate.'" class="check" />';
					$crbs_mon = 1;
					
				} /* else {
					#$out = str_replace('{tickbox}', '&nbsp;', $out);
					#$crbs_mon = 0;
				}*/
			...
			if($weekdates != NULL && array_key_exists($crbs_m, $weekdates)){
				...
				$out = str_replace('{rowclass}', '', $out);
			}
			
			
			// CR 2009-01-24
			// See if we had a valid monday this month week
			if($crbs_mon == 1){
				// Replace tickbox place-holder with actual one set before
				$out = str_replace('{tickbox}', $crbs_tick, $out);
				// Also put labels in the other days to make them clickable
				$out = str_replace('{label}', '<label for="chk_' . $crbs_mondate . '">', $out);
				$out = str_replace('{/label}', '</label>', $out);
			} else {
				// No tickbox and no labels
				$out = str_replace('{tickbox}', '&nbsp;', $out);
				$out = str_replace('{label}', '<span>', $out);
				$out = str_replace('{/label}', '</span>', $out);
				$out = str_replace('{rowclass}', '', $out);
			}
	...
	/**
	 * Generate the calendar for the bookings date selection
	 *
	 * @access	public
	 * @param	integer	the year
	 * @param	integer	the month
	...
	function generate_sidebar($year = '', $month = '', $academic, $cur = NULL, $urlpart = 'week')
	{
		...
		if ($this->show_next_prev == TRUE)
		{
			...
			// CR 2009-11-01. Is this new month in the list for this academic year?
			if(in_array($adjusted_date['year'].'/'.$adjusted_date['month'], $months)){			
				$out .= str_replace('{previous_url}', $this->next_prev_url.$adjusted_date['year'].'/'.$adjusted_date['month'], $this->temp['heading_previous_cell']);
				$out .= "\n";
			} else {
				$out .= $this->temp['heading_previous_cell_empty'];
				$out .= "\n";
			}
		...
		{		
			...
			if(in_array($adjusted_date['year'].'/'.$adjusted_date['month'], $months)){			
				...
				$out .= $this->temp['heading_next_cell_empty'];
				$out .= "\n";
			}
	 ...
	 */
 	function parse_template()
 	{
		...
		foreach (array('table_open', 'table_close', 'heading_row_start', 'heading_previous_cell', 'heading_previous_cell_empty', 'heading_title_cell', 'heading_next_cell', 'heading_next_cell_empty', 'heading_row_end', 'week_row_start', 'week_day_cell', 'week_row_end', 'cal_row_start', 'cal_cell_start', 'cal_cell_content', 'cal_cell_no_content',  'cal_cell_blank', 'cal_cell_end', 'cal_row_end', 'cal_cell_start_today', 'cal_cell_content_today', 'cal_cell_no_content_today', 'cal_cell_end_today') as $val)
		{
