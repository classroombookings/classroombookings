...
class Weeks extends MY_Controller
{
	...
	public function __construct()
	{
		...
		$this->require_permission(Permission::SETUP_TIMETABLE_WEEKS);
	...
	public function index()
	{
		...
		$this->data['title'] = $this->data['showtitle'] = lang('week.timetable_weeks');
	...
	public function add()
	{
		$this->data['title'] = $this->data['showtitle'] = lang('week.add.title');
	...
	function edit($id)
	{
		...
		$this->data['title'] = $this->data['showtitle'] = lang('week.edit.title');
	...
	private function save_week($week_id = NULL)
	{
		...
		$this->form_validation->set_rules('name', 'lang:week.field.name', 'required|max_length[20]');
		$this->form_validation->set_rules('bgcol', 'lang:week.field.colour', "required|min_length[6]|max_length[7]");
		...
		if ($week_id) {
			if ($this->weeks_model->update($week_id, $data)) {
				$msg = sprintf(lang('week.update.success'), $data['name']);
				$flashmsg = msgbox('info', $msg);
			} else {
				$flashmsg = msgbox('error', lang('week.update.error'));
			}
		} else {
			if ($week_id = $this->weeks_model->insert($data)) {
				$msg = sprintf(lang('week.create.success'), $data['name']);
				$flashmsg = msgbox('info', $msg);
			} else {
				$flashmsg = msgbox('info', lang('week.create.error'));
			}
		}
	...
	function delete($id)
	{
		...
		if ($this->input->post('id')) {
			...
			$flashmsg = msgbox('info', sprintf(lang('week.delete.success'), $week->name));
			$this->session->set_flashdata('saved', $flashmsg);
			redirect('weeks');
		}
		...
		$this->data['title'] = sprintf(lang('week.delete.title'), html_escape($week->name));
